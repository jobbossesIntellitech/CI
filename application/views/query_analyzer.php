...
*/
	...
	@$db = mysql_connect("localhost","pixelexp_firstla","x?}4=K&2r8kM");
	// if(! $db){
		// echo "<b>Error1</b> : ".mysql_error(); die();
	// }
	
	@$dbn = mysql_select_db("pixelexp_firstlamedia", $db);
	// if(! $dbn){
...
<style type="text/css">
	body{color:#2F2F2F;width:100%;margin:0px;background-color:#f8f8f8;color:#4D4D4D;}
		#forum{width:900px;margin:30px auto;padding:20px;border:1px solid #AfAfAf;background-color:powderblue;}
		#buttons{float:left;padding:5px;border:1px solid red;background-color:#8BE5AF;}
		...
		#main_box{border:0px solid #d8d8d8;background-color:powderblue;padding:15px 20px;margin:20px auto;width:900px;}
		#box2{border:1px solid red;background-color:#C58BEF;padding:5px 5px;margin:20px auto;width:900px;}
...
if(isset($_POST['predefined']))
{
	...
	if(mysql_errno()) //if there is an error in sql query(result=0)
		...
		<?php }
			...
			{
				?>
				<tr bgColor="#EBC7F0" style="color:#000"><?php
			...
			<td width="60%">
				<table>
					...
					<tr>
						...
						<form name="frmQuery" action="<?php echo base_url();?>index.php/query/analyzer" method="POST" onSubmit="return Verify()">
